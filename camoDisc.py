...
class Discriminator(object):
    ...
    def build(self,images):
        with tf.variable_scope("Discriminator_" + self.name,reuse=self.has_built):
            # h0 = lrelu(batch_norm(conv2d(images, 16,name='d_h0_conv'),scope='d_h0_conv')) #128x128x16
            # h1 = lrelu(batch_norm(conv2d(h0,64,name='d_h1_conv'),scope='d_h1_conv')) #64x64x64
            # h2 = lrelu(batch_norm(conv2d(h1,256,name='d_h2_conv'),scope='d_h2_conv')) #32x32x256
            # h3 = lrelu(batch_norm(conv2d(h1,1024,name='d_h3_conv'),scope='d_h3_conv')) #16x16x1024
            ...
            h3 = lrelu(batch_norm(conv2d(h2,64,name='d_h3_conv'),scope='d_h3_conv')) #16x16x64
            h4 = linear(tf.reshape(h3, [self.batch_size, -1]), 1, 'd_h4_lin')

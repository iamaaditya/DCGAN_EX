...
class DCGAN(object):
    def __init__(self, sess, ex, image_size=108, is_crop=True,
                 ...
                 checkpoint_dir=None, sample_dir=None, many=2):
        """
    ...
    def build_model(self):
        ...
        if self.y_dim:
            for x xrange(2):
                self.G[x] = self.generator(self.z, self.y)
                self.D[x], self.D_logits[x]  = self.discriminator(self.images, self.y, reuse=False)

                self.sampler[x] = self.sampler(self.z, self.y)
                self.D_[x], self.D_logits_[x] = self.discriminator(self.G[x], self.y, reuse=True)
        else:
            self.G[x] = self.generator(self.z)
            self.D[x], self.D_logits[x] = self.discriminator(self.images)

            self.sampler = self.sampler(self.z)
            self.D_[x], self.D_logits_[x] = self.discriminator(self.G[x], reuse=True)
        

        self.d_sum[x] = tf.histogram_summary("d"+str([x]), self.D[x])
        self.d__sum[x] = tf.histogram_summary("d_"+str([x]), self.D_[x])
        self.G_sum[x]= tf.image_summary("G"+str([x]), self.G[x])

        self.d_loss_real[x] = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits[x], tf.ones_like(self.D[x])))
        self.d_loss_fake[x] = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits_[x], tf.zeros_like(self.D_[x])))
        self.g_loss[x] = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(self.D_logits_[x], tf.ones_like(self.D_[x])))

        self.d_loss_real_sum[x] = tf.scalar_summary("d_loss_real"+str([x]), self.d_loss_real[x])
        self.d_loss_fake_sum[x] = tf.scalar_summary("d_loss_fake"+str([x]), self.d_loss_fake[x])
                                                    
        self.d_loss = self.d_loss_real[x] + self.d_loss_fake[x]

        self.g_loss_sum = tf.scalar_summary("g_loss"+str([x]), self.g_loss)
        self.d_loss_sum = tf.scalar_summary("d_loss"+str([x]), self.d_loss)

        t_vars = tf.trainable_variables()

        self.d_vars[x] = [var for var in t_vars if 'd_' in var.name and str(x) in var.name]
        self.g_vars[x] = [var for var in t_vars if 'g_' in var.name and str(x) in var.name]
    ...
    def train(self, config):
        ...
        d_optim[x] = tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1).minimize(self.d_loss[x], var_list=self.d_vars[x])
        g_optim[x] = tf.train.AdamOptimizer(config.learning_rate, beta1=config.beta1).minimize(self.g_loss[x], var_list=self.g_vars[x])
        tf.initialize_all_variables().run()

        self.g_sum[x] = tf.merge_summary([self.z_sum, self.d__sum[x], self.G_sum[x], self.d_loss_fake_sum[x], self.g_loss_sum[x]])
        self.d_sum = tf.merge_summary([self.z_sum[x], self.d_sum[x], self.d_loss_real_sum[x], self.d_loss_sum[x]])
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, batch_idxs):
                ...
                if config.dataset == 'mnist':
                    # Update D network
                    _, summary_str = self.sess.run([d_optim[x], self.d_sum[x]], feed_dict={self.images: batch_images, self.z: batch_z, self.y:batch_labels})
                    self.writer.add_summary(summary_str, counter)

                    # Update G network
                    _, summary_str = self.sess.run([g_optim[x], self.g_sum[x]], feed_dict={self.z: batch_z, self.y:batch_labels})
                    self.writer.add_summary(summary_str, counter)
